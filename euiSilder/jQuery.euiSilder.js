...
!(function(window) {
  ...
  Silder.pt.init = function() {
    ...
    _this.options.value =
      _this.$inputNode.val() ||
      (_this.options.isRange
        ? _this.options.from + ',' + _this.options.from
        : _this.options.from)
  ...
  Silder.pt.circleMove = function() {
    ...
    _this.$pointers.on('mouseleave', function(event) {
      if (_this.options.showLabels) _this.$wrapnum[0].style.display = 'none'
  ...
  Silder.pt.onDragStart = function(e) {
    ...
    $(document).on(
      ...
      $.proxy(_this.onDrag, this, pointer)
    )
    $(document).on(
      'mouseup.slider touchend.slider touchcancel.slider',
      $.proxy(_this.onDragEnd, this)
    )
